...
class ApplicationBase extends React.Component {
  ...
  componentDidMount() {
    ...
    if (firebase) {
      if (firebase.state) {
        if (firebase.state.data) {
          if (firebase.state.data.campTimes) {
            if (firebase.state.data.campTimes.length > 0) {
              this.getStateFromFirebase(this.props);
    ...
    if(firebase) {
      if(firebase.state) {
        if(firebase.state.data) {
          if(firebase.state.data.campTimes) {
            if(firebase.state.data.campTimes.length > 0) {
              if(firebase.state.data.campTimes !== state.campTimes) {
                console.log('getDerivedStateFromProps', props, state)
                let { campTimes, rawCampTimes, weekArray, yearChosen, localTimezoneOffset } = props.firebase.state.data;
                let loaded = true;
                let newState = {campTimes, rawCampTimes, weekArray, yearChosen, localTimezoneOffset};
  ...
  getStateFromFirebase = (props) => {
    let data = this.props.firebase.state.data;
    let functions = this.props.firebase.state.firebase;
    let { campTimes, rawCampTimes, weekArray, yearChosen, localTimezoneOffset, views, views2 } = data;
    this.setState({ campTimes, rawCampTimes, weekArray, yearChosen, localTimezoneOffset, views, views2 }, () => {
      console.log('apply mounted with firebase', this.state);
    });
  }
  componentDidUpdate(prevProps, prevState) {
  }
  ...
  makeRedirectString = (paymentMethod, window) => {
    ...
    if (this.props.location.pathname !== undefined) {
      redirectString = this.props.location.pathname.slice(0, (this.props.location.pathname.indexOf('/apply') + 1));
  ...
  render() {
    let { rawCampTimes, chosenYear, localTimezoneOffset, location, weekArray, campTimes } = this.state;
    return (
      // !this.loaded?
      //   null
      //   :
...
const Application = withFirebase(ApplicationBase);
