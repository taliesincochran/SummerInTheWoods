...
class Admin extends React.Component {
    ...
    handleSubmit = e => {
        e.preventDefault();
        if(this.state.username === `${process.env.ADMIN_USERNAME}` && `${this.state.process.env.ADMIN_PASSWORD}`) {
    ...
    getApplications = () => {
        const applicationsRef = db.getApplications();
        let applications = applicationsRef.then(snapshot => {
    ...
    handleApprove = (array, target, i, approve) => {
        ...
        let appRef = db.ref('applications/' + {target});
        let application = appRef.once('value');
        ...
        weekArray.forEach((week, i) => {
            if (week) {
                ...
                let week = weekRef.once('value').then(snapshot=> snapshot.val());
                let attending = weekRef.child('attending').once('value').then(snapshot=> snapshot.val());
        ...
        return (
            <div>
                ...
                : */}
                    ...
                    <div className="inner" style={this.state.loggedIn?{}:{display:'none'}}>
                        <h2>Applications</h2>
                        <div>
                            <Checkbox
                                style={{ display: "inline-block" }}
                                name="new"
                                checked={this.state.selected === "newApplications"}
                                value={true}
                            ...
                            <Checkbox
                                style={{ display: "inline-block" }}
                                name="approved"
                                checked={this.state.selected === "approvedApplictions"}
                                value={true}
                        ...
                        <div>
                            {/* (this.state.selected === 'new' && this.state.newApplications.length > 1 && Array.isArray(this.state.newApplications)) ?
                                <div>
                            ...
                            : (this.state.selected === 'approved' && this.state.approvedApplications.length > 1 && Array.isArray(this.state.approvedApplications)) ?
                                <div>
                            ...
                            : this.state.rejectedApplications.length > 1 && Array.isArray(this.state.rejectedApplications)?
                                <div>
